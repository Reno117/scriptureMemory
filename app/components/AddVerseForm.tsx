  ...
  const fetchESVText = async () => {
    ...
    try {
      ...
      if (data.text) {
        ...
        setFetchError(
          "Could not fetch verse. Check the reference and try again.",
        );
      }
    } catch {
    ...
    <div className="max-w-lg mx-auto px-4 py-12">
      ...
      >
        <div>
          ...
          <BookCombobox
            value={form.book}
            onChange={(val) => setForm((prev) => ({ ...prev, book: val }))}
          />
        </div>
        ...
        <div>
          <label className="text-xs text-stone-500 uppercase tracking-widest">
            Verse Text{" "}
            {form.translation === "ESV" ? "(auto-filled for ESV)" : ""}
          </label>
          <textarea
            name="text"
            value={form.text}
